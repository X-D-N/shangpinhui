          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{active:saleAttr.isChecked=='1'}" 
                  v-for="saleAttr in spuSaleAttr.spuSaleAttrValueList" :key="saleAttr.id"
                  @click="changeChecked(saleAttr,spuSaleAttr.spuSaleAttrValueList)">{{saleAttr.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="userInputShopCount($event)">
  ...
  export default {
    ...
    methods:{
      ...
      userInputShopCount(event) {
        //用户进来的文本 *1
        let value = event.target.value * 1;//获取用户输入的值
        // 不合法
      ...
      //添加到购物车  并判断请求是否是成功还是失败
      async addCart(){//由于是async所以返回的是promise
        /* 
        1、将参数带给服务器 通知服务器加入购物车的产品是谁
        ...
        try {
          ...
          //3、复杂数据(商品信息)存入sessionStorage  - - 便于添加购物车成功页面读取
          // 无论是本地存储还是会话存储 都存的是字符串
