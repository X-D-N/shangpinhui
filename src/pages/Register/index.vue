    ...
    methods:{
      ...
      async getCode(){
        try {
          ...
          const {phone} =this;
          //验证码自己填写
          phone&&(await this.$store.dispatch('user/sendCode',phone));
      ...
      // 用户注册
      async userRegister() {
        const success = await this.$validator.validateAll();//全部表单验证成功后 在向服务器发请求
        if (success) {
          try {
            //成功 进行路由跳转
            let { phone, code, password, password1 } = this;
            phone && code && password == password1 && await this.$store.dispatch("user/sendRegisterInfo", { phone, code, password });
